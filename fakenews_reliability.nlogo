...
turtles-own [
  ...
  reliable ;; random value between 0 and 1, higher the reliability the more likely we are to believe
  party ;; 1 or 2, if we are of the same partisanship we are more likely to believe, if we are not it is the same as reliability
]
...
to create-preferential-attachment
  nw:generate-preferential-attachment turtles links 500 1 [
    ...
    set color blue
    set adopted? false
    ifelse reliability [set reliable random-float 1] [ set reliable 1 ] ;;set reliability to random value between 0 and 1
    set party 1 + random 2
  ]
end
...
to decide-to-adopt
  ifelse random-float 1.0 < 0.01 [
    ...
    if any? link-neighbors [
      ...
      ifelse reliability
      [
        let avg-reliability 1
        let temp sum [reliable] of link-neighbors / count link-neighbors
        ifelse 0.5 > temp
        [
          set avg-reliability 1 + temp
        ]
        [
          set avg-reliability temp
        ]
        ifelse partisanship
        [
          let neighbor-partisans (count link-neighbors with [party = [party] of myself] / count link-neighbors) + 1
          if random-float 1.0 < 0.5 * neighbors-adoption * avg-reliability * neighbor-partisans [
            set adopted? true
          ]
        ]
        [
          if random-float 1.0 < 0.5 * neighbors-adoption * avg-reliability [
            set adopted? true
          ]
        ]
      ]
      [
        ...
        [
          let neighbor-partisans (count link-neighbors with [party = [party] of myself] / count link-neighbors) + 1
          if random-float 1.0 < 0.5 * neighbors-adoption * neighbor-partisans [
            set adopted? true
          ]
        ]
        [
